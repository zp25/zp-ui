 ...
 * @ignore
 */
function carouselBase(group) {
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  var focus = Number(options.focus) || 1;
  var delay = options.delay || 8000; // 组

  this.group = group || ''; // 主要元素

  this.carousel = $(".carousel".concat(group ? "[data-group=\"".concat(group, "\"]") : ''));
  this.main = this.carousel.children('.carousel__main'); // 初始聚焦页，没有上边界判断
...
carouselBase.prototype.mainSwitch = function mainSwitch(next) {
  ...
  this.main.data('focus', next).animate({
    left: offset
  }, 500);
};
/**
...
carouselBase.prototype.play = function play() {
  ...
  var focus = this.main.data('focus');
...
function Carousel(group) {
  ...
  var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  carouselBase.call(this, group, options); // 主要元素

  this.nav = this.carousel.children('.carousel__nav'); // 是否自动播放

  this.isAutoplay = false; // 绑定事件监听

  this.nav.on('click', function (e) {
    e.preventDefault();
    var next = Number($(e.target).data('order'));

    if (next && next > 0) {
      _this.combineSwitch(next);
    }
  });
}
...
Carousel.prototype.navSwitch = function navSwitch(next) {
  this.nav.children('.slide-nav').removeClass('slide-nav--active').filter("[data-order=\"".concat(next, "\"]")).addClass('slide-nav--active');
};
/**
 * 完成切换任务
 * @param {number} next - 下一页编号
...
Carousel.prototype.combineSwitch = function combineSwitch(next) {
  this.clearTimeout(); // main样式、记录修改

  Object.getPrototypeOf(Carousel.prototype).mainSwitch.call(this, next); // nav样式修改
...
Carousel.prototype.play = function play(reverse) {
  var next = Object.getPrototypeOf(Carousel.prototype).play.call(this, reverse);
...
CarouselLite.prototype.play = function play() {
  this.clearTimeout();
  var next = Object.getPrototypeOf(CarouselLite.prototype).play.call(this);
